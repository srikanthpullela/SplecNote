# SplecNote — Complete Project Reference
...
```bash
cd "/Users/spullela/AI Projects/SplecNote"
...
├── .gitignore
├── README.md
├── SPLECNOTE.md              # THIS FILE — full project reference
├── build/
│   ├── icon.icns             # macOS app icon
│   ├── icon.ico              # Windows app icon
│   └── icon.png              # PNG source icon
├── dist/                     # Build output (generated)
│   ├── SplecNote-1.0.0-arm64.dmg       # macOS installer (~106 MB)
│   ├── SplecNote-1.0.0-arm64-mac.zip   # macOS zip (~103 MB)
...
```bash
cd "/Users/spullela/AI Projects/SplecNote" && source ~/.nvm/nvm.sh && nvm use 22 && npx electron . &
```

**To kill and restart:**

```bash
pkill -f "Electron.*SplecNote" 2>/dev/null; sleep 1
cd "/Users/spullela/AI Projects/SplecNote" && source ~/.nvm/nvm.sh && nvm use 22 && npx electron .
```
...
**Output** (in `dist/`):
- `SplecNote-1.0.0-arm64.dmg` — macOS disk image installer (~106 MB)
- `SplecNote-1.0.0-arm64-mac.zip` — Compressed app (~103 MB)
...
- **Data paths**: `~/SplecNote/.session.json`, `~/SplecNote/.recent.json`, `~/SplecNote/AutoSave/`

### `src/preload/preload.js` (69 lines)

Secure IPC bridge. Exposes `window.splecnote` with:
...
- **14 theme definitions** as CSS custom properties (e.g., `[data-theme="dark"]`, `[data-theme="splec"]`)
...
- **Auto-save** — Unsaved content auto-saved to `~/SplecNote/AutoSave/`
...
### Themes (14)
- Dark (Default), Light, Monokai, Dracula, Nord, Solarized Dark, Sublime Mariana, One Dark Pro, Material Ocean, GitHub Dark, Tomorrow Night, Ayu Dark, Gruvbox Dark, **Splec Theme**
...
| `splec` | Splec Theme | vs-dark |

**Splec Theme** uses the Monokai color palette (`#272822` bg, `#f8f8f2` fg) with ~60 token rules including:
...
All app data is stored in `~/SplecNote/`:
...
- **Auto-save** (`scheduleAutoSave()`): Debounced 2-second timer on content change. Saves to `~/SplecNote/AutoSave/<filename>`.
...
### App shows "Electron" in menu bar instead of "SplecNote"
- Fixed: `app.setName('SplecNote')` is called before anything else in `main.js`
...
- Or: `xattr -cr "/path/to/SplecNote.app"`
...
| Splec theme token rules | inside defineMonacoThemes |
| `initMonaco()` | 475 |
| `createTab()` | 517 |
| `activateTab()` | 549 |
...
*This file is the single source of truth for the SplecNote project. Keep it updated when making changes.*
