# CongaNote — Complete Project Reference
...
```bash
cd "/Users/spullela/AI Projects/CongaNote"
...
├── .gitignore
├── README.md
├── CONGANOTE.md              # THIS FILE — full project reference
├── build/
│   ├── icon.icns             # macOS app icon
│   ├── icon.ico              # Windows app icon
│   └── icon.png              # PNG source icon
├── dist/                     # Build output (generated)
│   ├── CongaNote-1.0.0-arm64.dmg       # macOS installer (~106 MB)
│   ├── CongaNote-1.0.0-arm64-mac.zip   # macOS zip (~103 MB)
...
```bash
cd "/Users/spullela/AI Projects/CongaNote" && source ~/.nvm/nvm.sh && nvm use 22 && npx electron . &
```

**To kill and restart:**

```bash
pkill -f "Electron.*CongaNote" 2>/dev/null; sleep 1
cd "/Users/spullela/AI Projects/CongaNote" && source ~/.nvm/nvm.sh && nvm use 22 && npx electron .
```
...
**Output** (in `dist/`):
- `CongaNote-1.0.0-arm64.dmg` — macOS disk image installer (~106 MB)
- `CongaNote-1.0.0-arm64-mac.zip` — Compressed app (~103 MB)
...
- **Data paths**: `~/CongaNote/.session.json`, `~/CongaNote/.recent.json`, `~/CongaNote/AutoSave/`

### `src/preload/preload.js` (69 lines)

Secure IPC bridge. Exposes `window.conganote` with:
...
- **Auto-save** — Unsaved content auto-saved to `~/CongaNote/AutoSave/`
...
All app data is stored in `~/CongaNote/`:
...
- **Auto-save** (`scheduleAutoSave()`): Debounced 2-second timer on content change. Saves to `~/CongaNote/AutoSave/<filename>`.
...
### App shows "Electron" in menu bar instead of "CongaNote"
- Fixed: `app.setName('CongaNote')` is called before anything else in `main.js`
...
- Or: `xattr -cr "/path/to/CongaNote.app"`
...
*This file is the single source of truth for the CongaNote project. Keep it updated when making changes.*
